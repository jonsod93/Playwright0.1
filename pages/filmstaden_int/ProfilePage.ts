...
export class ProfilePage extends DefaultPageWithNavigation {
    ...
    constructor(page) {
        ...
        this.communicationAccordion = this.page.getByRole('button', { name: ' Kundkommunikation' });
        this.friendsAccordion = this.page.getByRole('button', { name: ' Poängdelning med vänner' });
        this.remindersCheckbox = this.page.getByRole('checkbox', { name: 'Påminnelser, aktuella hä' });
        this.competitionsCheckbox = this.page.getByRole('checkbox', { name: 'Inbjudningar och tävlingar' });
        this.ticketReleasesCheckbox = this.page.getByRole('checkbox', { name: 'Biljettsläpp och erbjudanden' });
        this.membershipInformationCheckbox = this.page.getByRole('checkbox', { name: 'Medlemsinformation och' });
        this.friendUserIDField = this.page.getByRole('textbox', { name: 'Medlemsnummer' });
        this.friendNameField = this.page.getByRole('textbox', { name: 'Ange ett namn' });
        this.addFriendButton = this.page.getByRole('button', { name: 'Lägg till ny vän' });
        this.cancelMembershipButton = this.page.getByRole('button', { name: 'Avsluta medlemskap' });
        this.confirmCancelButton = this.page.getByRole('button', { name: 'Ja, avsluta mitt konto' });
        this.QRCodeInfoLocator = this.page.getByRole('heading', { name: 'Din enhets skärmljusstyrka', exact: true });
    ...
    async navigateToProfilePage(url: string) {
        await this.page.goto(url+'mina-sidor/');
    ...
    async getUserID(): Promise<string> {
        ...
        return userID.trim(); // Ensure it returns a trimmed string
    }
