...
export class SignUpFlowPage extends DefaultPage {
    ...
    constructor(page) {
      ...
      this.confirmPasswordField = this.page.getByPlaceholder('Bekräfta lösenordet');
      this.sendCodeButton = this.page.getByRole('button', { name: 'Skicka verifieringskod' });
      this.verificationCodeField = this.page.getByPlaceholder('Verifieringskod');
      this.verifyButton = this.page.getByRole('button', { name: 'Verifiera koden' });
    ...
    async getSocialSecurityNumber(): Promise<string> {
        ...
        try {
            ...
            const consentButton = socialSecurityNumberPage.getByLabel('Jag samtycker');
