...
test.describe.parallel('Discount code purchases', () => {
  ...
  test('Discountcode FR purchase + control the confirmation email', async ({
    page,
  }, testInfo) => {
    await test.step('Initial setup of test specific variables and tags', async () => {
      ...
      testEmailAddress =
        ...
        `FRpurchase@${namespace}.mailisk.net`;
    ...
    await test.step('Verify discount error message and add a correct discount code', async () => {
      ...
      await expect(showPage.discountCodeInfo).toContainText(
        'Endast ett erbjudande per biljett'
      );

      await showPage.fillDiscountCode(DiscountCodes.invalid.code);
      await showPage.activateDiscountCode();
      await expect(showPage.formLocator).toContainText(
        'Rabatten hittades inte'
      );
      await showPage.fillDiscountCode(DiscountCodes.free.code);
      await showPage.activateDiscountCode();
      await expect(showPage.mainContentLocator).toContainText(
        DiscountCodes.free.pagePriceAfterDiscount
      );
      await expect(showPage.mainContentLocator).toContainText(
        'Ingen betalnings behövs. Njut av din filmupplevelse!'
      );
    });
    ...
    await test.step('Finish the purchase and control tickets', async () => {
      ...
      await expect(receiptPage.mainContentLocator).toContainText(
        'Antal biobiljetter:'
      );
    ...
    await test.step('Verify the verification email', async () => {
      ...
      const emailContent = await emailHelper.getEmailContent(
        testEmailAddress,
        timeFrom,
        namespace
      );
      ...
      await expect(emailContent).toContain(
        DiscountCodes.free.mailPriceAfterDiscount
      );
    });
  });
});
