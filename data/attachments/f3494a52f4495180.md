...
Call log:
  - waiting for getByRole('button', { name: 'Fortsätt till kortbetalning' })
    - locator resolved to <button type="button" class="relative *:focus-visible:outline *:focus-visible:outline-2 *:focus-visible:outline-offset-2 disabled:cursor-not-allowed duration-200 transition-colors ease-out  rounded-full *:rounded-full font-bold h-[2.75rem] text-base bg-brand text-button-primary border-0 hover:bg-brand-strong focus-visible:bg-brand focus-visible:outline-none active:text-button-primary-active active:bg-brand-stronger disabled:bg-disabled disabled:text-disabled w-full ">…</button>
